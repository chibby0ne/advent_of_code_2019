...
fn main() {
    ...
    let intersections = find_intersections(&first_wire_path, &second_wire_path);
    println!(
        "answer is: {}",
        find_minimum_manhattan_distance_from_origin(&intersections)
    );
}

fn find_minimum_manhattan_distance_from_origin(vec: &[Point]) -> i64 {
    let origin = Point::new();
    let min = vec
        .iter()
        .min_by(|a, b| manhattan(&origin, a).cmp(&manhattan(&origin, b)))
        .unwrap();
...
fn find_intersections(v1: &[Point], v2: &[Point]) -> Vec<Point> {
    let origin = Point::new();
